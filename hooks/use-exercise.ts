...
import { loadExercises, getExerciseById, LEGACY_EXERCISES } from '@/lib/mock-data/exercises';
...
export function useExerciseLookup() {
  ...
  useEffect(() => {
    ...
    loadExercises()
      ...
      .catch((err) => {
        if (!cancelled) {
          // Fallback to legacy exercises on error
          setExercises(LEGACY_EXERCISES);
          setError(err);
          setIsLoading(false);
        }
      });
    ...
    loadExercises()
      ...
      .catch((err) => {
        if (!cancelled) {
          ...
          const idsSet = new Set(exerciseIds);
          const map = new Map<string, Exercise>();
          for (const ex of LEGACY_EXERCISES) {
            if (idsSet.has(ex.id)) {
              map.set(ex.id, ex);
            }
          }
