...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!packageId || !trainerId || !clientId || !amountCents) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: packageName || 'Session Package',
              description: `${sessionCount} training sessions`,
            },
          },
          quantity: 1,
        },
      ...
      success_url: successUrl || `${origin}/client/packages?purchase=success`,
      cancel_url: cancelUrl || `${origin}/client/packages?purchase=cancelled`,
      metadata: {
    ...
    await supabase.from('ta_payments').insert({
      ...
      description: `${packageName || 'Session Package'} (${sessionCount} sessions)`,
    });
