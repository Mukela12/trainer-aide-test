...
/**
 * GET /api/ai-programs/assigned
 * Fetch AI programs assigned to the current authenticated user (trainer)
 */
export async function GET() {
  try {
    ...
    // Fetch AI programs assigned to this user
    ...
    if (error) {
      console.error('Error fetching assigned AI programs:', error);
      return NextResponse.json(
        { error: 'Failed to fetch assigned AI programs' },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ templates: aiPrograms });
  } catch (error) {
    console.error('Error in assigned AI programs GET:', error);
